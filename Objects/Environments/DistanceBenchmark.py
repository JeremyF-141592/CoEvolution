...
class DistanceBenchmarkEnv(Environment):
    """
    ...
    def get_child(self):
        child = DistanceBenchmarkEnv(self.dim, self.specialist_fit, self.generalist_fit, self.generalist_points, self.bounds)
...
class DistanceBenchmarkEnvFactory(EnvironmentFactory):
    ...
    def new(self):
        return DistanceBenchmarkEnv(self.dim, self.spec_fit, self.gen_fit, self.generalist_points, self.bounds)


class DistanceBenchmarkAg(Agent):
...
class DistanceBenchmarkAgFactory(AgentFactory):
    def __init__(self, dimension, bounds):
        self.dim = dimension
        self.bounds = bounds

    def new(self):
        return DistanceBenchmarkAg(self.dim, self.bounds)
