...
parser.add_argument('--max_env_children', type=int, default=50, help='Maximum number of env children per reproduction')
...
def NSGAII_ag_env(pop, envs, additional_objectives, args):
    ...
    obs = [list() for i in range(len(new_evs))]
    for i in range(len(new_evs)):
        print(f"\tEnvironment {i}", flush=True)
        res = Configuration.lview.map(new_evs[i], new_pop)
    ...
    results = add_objectives(fit, obs, new_pop, additional_objectives, new_evs, args)
