...
class CppnEnvParams:
    ...
    def __init__(self, cppn_config_path='config-cppn', genome_path=None):
        ...
        self.hardcore = False
        self.cppn_config = neat.Config(neat.DefaultGenome, neat.DefaultReproduction, neat.DefaultSpeciesSet,
                                       neat.DefaultStagnation, self.cppn_config_path)
    ...
    def get_child(self):
        is_valid = False
        while not is_valid:
            mutated = pickle.loads(pickle.dumps(self.cppn_genome))  # Copy
            mutated.nodes[0].response = 1.0
            mutated.key = datetime.datetime.utcnow().isoformat()
            mutated.mutate(self.cppn_config.genome_config)
            is_valid = is_genome_valid(mutated) & \
                       (self.cppn_genome.distance(mutated, self.cppn_config.genome_config) > 0)
            if not is_valid:
                continue
    ...
    def draw(self, arr):
        ...
        for i in range(len(arr)):
            res[i] = self.altitude_fn([arr[i]])[0]
        return res
